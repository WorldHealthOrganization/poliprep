...
validate_date <- function(data, date_col, 
                          tests = c("missing", "non_date", "sensible", 
                                    "leap_year", "format"), 
                          min_year = 2000) {
...
validate_dates <- function(data, date_col1, date_col2, 
  ...
  if ("sensible" %in% tests) {
    ...
    invalid_date1 <- !between(lubridate::year(
      as.Date(data[[date_col1]], format = "%Y-%m-%d")), min_year, current_year)
    invalid_date2 <- !between(lubridate::year(
      as.Date(data[[date_col2]], format = "%Y-%m-%d")), min_year, current_year)
    
    future_date1 <- lubridate::year(
      as.Date(data[[date_col1]], format = "%Y-%m-%d")) > current_year
    future_date2 <- lubridate::year(
      as.Date(data[[date_col2]], format = "%Y-%m-%d")) > current_year
