testthat::test_that("correct_flipped_geo_coords works correctly", {
  ...
  data <- data.frame(
    id = c(
      "{0CDA1C45-9529-4188-BE2F-863E287EDA71}",
  ...
  testthat::expect_equal(
    round(result$lat), c(41, 41),
    tolerance = 1e-4
  )
  testthat::expect_equal(
    round(result$lon), c(-74, -74),
    tolerance = 1e-4
  )
  ...
  # Check if coordinates are corrected
  testthat::expect_equal(
    round(result2$lat), c(41, 41),
    tolerance = 1e-4
  )
  testthat::expect_equal(
    round(result2$lon), c(-74, -74),
    tolerance = 1e-4
  )
  ...
  testthat::expect_error(
    ...
    paste0(
      "The join key 'non_existent_key' is not ",
  ...
  testthat::expect_error(
    ...
    paste0(
      "The column 'correct_lat2' is not present in ",
  ...
  testthat::expect_error(
    ...
    paste0(
      "The column 'correct_lon2' is not present in ",
      "the shapefile_data dataframe."
    )
  )
  ...
  # Check if coordinates are corrected
  testthat::expect_equal(
    round(res$lat), c(41, 42),
    tolerance = 1e-4
  )
  testthat::expect_equal(
    round(res$lon), c(-74, -88),
    tolerance = 1e-4
  )
})
...
testthat::test_that("check_land_water function works correctly", {
  ...
  test_coords <- data.frame(
    lon = c(
      -74.1613, -70.7210, 32.9333, 22.8465,
      -0.6796, -0.0420, NA, 45.5017, -180, 180
    ),
    lat = c(
      40.7772, 40.2987, -1.1466, 1.7656,
